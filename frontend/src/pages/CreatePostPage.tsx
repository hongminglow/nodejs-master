...
export default function CreatePostPage() {
	...
	const [createPost, { loading, error }] = useMutation<{ createPost: { id: string } }>(CREATE_POST, {
		onCompleted: (data) => {
	...
	return (
		<div className="max-w-3xl mx-auto animate-fade-in">
			...
			<h1 className="text-3xl sm:text-4xl font-extrabold text-surface-50 leading-tight mb-6">Create a New Post</h1>
			...
			<div className="glass-card rounded-2xl p-6 sm:p-8">
				{(error || formError) && (
					<div className="flex items-start gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/20 mb-6 animate-fade-in">
						...
						<p className="text-sm text-red-300">{formError || error?.message}</p>
					</div>
				)}
				...
				<form onSubmit={handleSubmit} className="space-y-6">
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
						...
						<div>
							<label htmlFor="status" className="block text-sm font-medium text-surface-300 mb-2">
								Status
							</label>
							<select
								id="status"
